...
export default function eventsDatepicker() {
  const elements = Array.from(
    document.querySelectorAll<HTMLElement>(".js-events-datepicker")
  );
  ...
  elements.forEach((element) => {
    ...
    const datesText = element.querySelector<HTMLElement>(
      ".events__filters-datepicker-dates-text"
    );
    ...
    const dates = element.querySelector<HTMLElement>(
    ...
    const instance = new AirDatepicker(input, {
      ...
      onSelect: ({ formattedDate }) => {
        const joinedDate = Array.isArray(formattedDate)
          ? formattedDate.join(" - ")
          : formattedDate;
        ...
        if (!joinedDate.trim().length) {
          datesText.textContent = "Выберите день или период ";
  ...
  <svg width="30" height="30" viewBox="0 0 30 30" xmlns="http://www.w3.org/2000/svg">
    ...
    dates?.addEventListener("click", () => {
      document.body.classList.add("mobile-datepicker");
    });

    closeBtns.forEach((btn) =>
      btn.addEventListener("click", () => {
        document.body.classList.remove("mobile-datepicker");
      })
    );
